...
After the `Exec` attempt—whether it returns or throws—a bounded HTA timer checks **only** the three bridge files every 100 ms. It never inspects the process object. The preferred result is `PASS` with `bridge_done.txt=SUCCESS`. For compatibility, a nonempty result whose file size remains unchanged for at least 300 ms and contains a complete valid record may produce `PASS_COMPAT` while `BRIDGE_DONE` remains `MISSING`. This avoids reading a partially written result and supports the observed return-object failure. The same stable-file fallback recognizes the proven color-key result record.

A `bridgeBusy` flag refuses overlaps until a marker/fallback is processed or 20 seconds elapse. At timeout, a stable valid result is `PASS_COMPAT`, a PowerShell error file is `FAILED`, no bridge files is `BLOCKED_OR_NO_LAUNCH`, and another incomplete-file case is `TIMEOUT`. The flag is then cleared; the PoC deliberately does not attempt unreliable process termination. Operations are therefore intentionally short and bounded. There is no continuous window enumeration or persistent bridge architecture.
...
13. Select **Clear diagnostic errors**, then **Test PowerShell window bridge** once. Confirm exactly one invocation is added. Prefer `BRIDGE_DONE: SUCCESS` with `POWERSHELL_BRIDGE: PASS`; if the done marker is missing, accept `PASS_COMPAT` only with `BRIDGE_RESULT_FILE: FOUND` and a stable `VALID_WINDOW_ROWS` greater than zero. `EXEC_RETURN_ERROR` may contain the known Korean COM message, while `BRIDGE_ERROR` must remain `none` unless PowerShell wrote its error file. Confirm the table contains credible HWND/title/rectangle rows and no raw `#< CLIXML` appears. If it fails, copy both distinct error fields and inspect the three bridge text files under `data`. Clicking a PowerShell action again while this request is busy must not launch another process.
